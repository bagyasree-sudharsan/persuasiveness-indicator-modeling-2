...
def evaluate_final_model(model_path, dataset_path, evaluation_title, num_labels):
    ...
    # id2label_argcomps = {i: v for i, v in enumerate(TEXT_SEG_ARG_COMP_TAGS)}
    # id2label_semtypes = {i: v for i, v in enumerate(SEMANTIC_TYPE_TAGS)}
    # arg_pairs_with_tags = [(add_tags_to_text([arg1_list[i]], id2label_argcomps, id2label_semtypes), add_tags_to_text([arg2_list[i]]), id2label_argcomps, id2label_semtypes) for i in range(0, len(arg_pairs))]
...
model_path = 'models/BaselineRegressorFixed'
dataset_path = 'datasets/processed/AnnotatedCMV/final.json'
evaluate_model(model_path, dataset_path, 'Baseline AnnotatedCMV on AnnotatedCMV - Regression Fixed', 1)
