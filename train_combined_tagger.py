...
labels = [d['combined_tags'] for d in dataset]
...
def compute_metrics(p):
    ...
    true_predictions = [
        [COMBINED_TAGS[p] for (p, l) in zip(prediction, label) if l != -100]
        for prediction, label in zip(predictions, labels)
    ]
    true_labels = [
        [COMBINED_TAGS[l] for (p, l) in zip(prediction, label) if l != -100]
        for prediction, label in zip(predictions, labels)
    ]
...
id2label =  {v: k for v, k in enumerate(COMBINED_TAGS)}

model = AutoModelForTokenClassification.from_pretrained("distilbert/distilbert-base-uncased", num_labels=25, id2label=id2label, label2id=label2id).to('cuda')

print('Loaded model.')

training_args = TrainingArguments(
    output_dir="combined_trainer",
    learning_rate=2e-5,
...
trainer.save_model('models/CombinedTagger')
print('Saved model CombinedTagger.')
